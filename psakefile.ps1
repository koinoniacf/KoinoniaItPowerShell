...
Task UpdatePSScriptInfo {
    ...
    $SourceScripts | ForEach-Object {
        ...
        if ($null -ne $GitStatus) {
            ...
            $Copyright = (Test-ScriptFileInfo  $_.FullName).Copyright -replace "^Copyright \(c\) Koinonia Christian Fellowship(.*)$", "Copyright (c) Koinonia Christian Fellowship $((Get-Date).Year)" -replace "^Copyright \(c\) Dennis Group(.*)$", "Copyright (c) Dennis Group $((Get-Date).Year)"
...
Task CompressModule { Compress-Archive -Path .\KoinoniaIT*.ps* -DestinationPath Release.zip -Update }
